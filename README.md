...
1. Вы вставляете длинную ссылку → сервер генерирует уникальный короткий ID (например, `abc123`)
2. Короткая ссылка выглядит так: `http://ваш-сайт/abc123`
3. Когда кто-то переходит по ней, сервер:
   - Ищет в базе оригинальную ссылку
   - Увеличивает счётчик переходов
   - Перенаправляет (редирект) на оригинальный сайт
4. В любой момент можно посмотреть статистику: сколько переходов было по ссылке
---
